...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        var userName: String = ""
        userName = UserDefaults.standard.object(forKey: "userDefaultsName") as? String ?? ""
        
        let navController: UINavigationController
        
        if userName.isEmpty {
            navController = UINavigationController(rootViewController: UserNameController() )
        } else {

            let main = MainScreenController()
            main.userName = userName
            navController = UINavigationController(rootViewController: main)
